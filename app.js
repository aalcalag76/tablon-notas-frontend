...
try {
    ...
    if (notas.length === 0) {
        contenedorNotas.innerHTML = '<p style="color: #666; text-align: center; gridcolumn: 1/-1;">No hay notas publicadas aún.</p>';
    return;
}
notas.forEach(nota => {
    ...
    tarjeta.innerHTML = `<div> <p class="nota-autor"> ${nota.nombre}</p><p class="nota-mensaje">${nota.mensaje}</p></div>
<div>
<div class="acciones">
<button class="btn-editar" onclick="prepararEdicion(${nota.id},
'${nota.nombre}', '${nota.mensaje}')"> Editar</button>
